describe('Edit Book Unit - Cancel', () => {
  ...
  const bookTitle = `Cypress Book For Unit Edit Cancel ${uniqueId}`;
  const unitData = {
    language: 'CancelLang',
    pageCount: '321',
  ...
  const newLanguage = 'ShouldNotSave';
  ...
  it('should add a new book and a new book unit', () => {
    ...
    cy.get('.dialog textarea').type('Book for unit edit cancel test.');
  ...
  it('should open the edit unit dialog and cancel', () => {
    cy.contains('Genres').click();
    cy.contains('.genre-card', 'Fantasy').click();
    cy.contains('.book-title', bookTitle)
      .parents('.book-card')
    ...
    cy.get('.dialog input').eq(0).clear().type(newLanguage);
    cy.get('.dialog button[type="button"]').contains('Cancel').click();
    cy.get('.dialog').should('not.exist');
    cy.get('.unit-card').last().should('contain.text', unitData.language);
    cy.get('.unit-card').last().should('not.contain.text', newLanguage);
  });
}); 