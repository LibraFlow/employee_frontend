describe('Book Management - Edit Book', () => {
  ...
  it('should edit the book description and verify the update', () => {
    ...
    cy.get('.dialog textarea').clear().type('This is an updated description by Cypress.');
    cy.get('.dialog button[type="submit"]').contains('Save Changes').click();
    cy.reload();
    cy.get('.book-card').filter((index, card) => {
      return card.querySelector('.book-title')?.textContent === bookTitle;
    }).last().find('.book-description')
      .should('contain', 'This is an updated description by Cypress.');
